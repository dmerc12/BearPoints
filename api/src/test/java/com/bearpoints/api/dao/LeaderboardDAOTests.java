...
public class LeaderboardDAOTests {
    ...
    @Mock
    private TypedQuery<LeaderboardEntryDTO> contentQuery;
    ...
    private List<LeaderboardEntryDTO> sampleResults;
    ...
    void setUp() {
        ...
        sampleResults = Arrays.asList(
                new LeaderboardEntryDTO(1,
                        ...
                        new PersonDTO(101L, "Jane", "Smith"),
                        "FIRST", 150),
                new LeaderboardEntryDTO(2,
                        new PersonDTO(2L, "Alice", "Johnson"),
                        new PersonDTO(102L, "Bob", "Brown"),
                        "SECOND", 120),
                new LeaderboardEntryDTO(3,
                        new PersonDTO(3L, "Charlie", "Wilson"),
                        new PersonDTO(101L, "Jane", "Smith"),
                        "FIRST", 100)
        );
    }
    ...
    void findRankedLeaderboard_NoFilters_ReturnsRankedResults() {
        when(entityManager.createQuery(anyString(), eq(LeaderboardEntryDTO.class))).thenReturn(contentQuery);
        ...
        assertEquals("FIRST", firstEntry.getGrade());
    ...
    void findRankedLeaderboard_WithTeacherFilter_ReturnsClassRankedResults() {
        ...
        when(contentQuery.setFirstResult(0)).thenReturn(contentQuery);
        when(contentQuery.setMaxResults(20)).thenReturn(contentQuery);
        List<LeaderboardEntryDTO> teacherResults = List.of(
    ...
    void findRankedLeaderboard_WithGradeFilter_ReturnsRankedResults() {
        ...
        when(contentQuery.setFirstResult(0)).thenReturn(contentQuery);
        when(contentQuery.setMaxResults(20)).thenReturn(contentQuery);
        List<LeaderboardEntryDTO> gradeResults = List.of(
                sampleResults.getFirst(),
                sampleResults.get(2)
        );
    ...
    void findRankedLeaderboard_WithTeacherAndGradeFilters_ReturnsFilteredResults() {
        ...
        when(contentQuery.setFirstResult(0)).thenReturn(contentQuery);
        when(contentQuery.setMaxResults(20)).thenReturn(contentQuery);
        List<LeaderboardEntryDTO> filteredResults = List.of(sampleResults.getFirst());
    ...
    void findRankedLeaderboard_WithPagination_ReturnsCorrectPage() {
        ...
        List<LeaderboardEntryDTO> secondPageContent = List.of(sampleResults.get(2));
