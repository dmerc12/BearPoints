...
public class AdminController {
     ...
     * @param size Page size (default: 20)
     * @param sort Sort criteria (default: lastName,asc
     * @return Paginated response of matching admin users
     */
    @GetMapping("/search/email")
    public ResponseEntity<PagedResponseDTO<UserDTO>> searchAdminsByEmail(
            @RequestParam String email,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(defaultValue = "lastName,asc") String sort
    ) {
        log.debug("Searching admin users by email: {} - page: {}, size: {}, sort: {}", email, page, size, sort);
        String[] sortParams = splitSortParams(sort);
        Sort.Direction direction = sortParams.length > 1 && "desc".equalsIgnoreCase(sortParams[1])
                ? Sort.Direction.DESC
                : Sort.Direction.ASC;
        Pageable pageable = PageRequest.of(page, size, Sort.by(direction, sortParams[0]));
        PagedResponseDTO<UserDTO> response = adminService.searchAdminsByEmail(email, pageable);
        log.info("Found {} admin users matching email: {}", response.getNumberOfElements(), email);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<PagedResponseDTO<UserDTO>> searchAdminsByFirstName(
            @RequestParam String firstName,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(defaultValue = "lastName,asc") String sort
    ) {
        log.debug("Searching admin users by first name: {} - page: {}, size: {}, sort: {}", firstName, page, size, sort);
        String[] sortParams = splitSortParams(sort);
        Sort.Direction direction = sortParams.length > 1 && "desc".equalsIgnoreCase(sortParams[1])
                ? Sort.Direction.DESC
                : Sort.Direction.ASC;
        Pageable pageable = PageRequest.of(page, size, Sort.by(direction, sortParams[0]));
        PagedResponseDTO<UserDTO> response = adminService.searchAdminsByFirstName(firstName, pageable);
        log.info("Found {} admin users matching first name: {}", response.getNumberOfElements(), firstName);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<PagedResponseDTO<UserDTO>> searchAdminsByLastName(
            @RequestParam String lastName,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(defaultValue = "lastName,asc") String sort
    ) {
        log.debug("Searching admin users by last name: {} - page: {}, size: {}, sort: {}", lastName, page, size, sort);
        String[] sortParams = splitSortParams(sort);
        Sort.Direction direction = sortParams.length > 1 && "desc".equalsIgnoreCase(sortParams[1])
                ? Sort.Direction.DESC
                : Sort.Direction.ASC;
        Pageable pageable = PageRequest.of(page, size, Sort.by(direction, sortParams[0]));
        PagedResponseDTO<UserDTO> response = adminService.searchAdminsByLastName(lastName, pageable);
        log.info("Found {} admin users matching last name: {}", response.getNumberOfElements(), lastName);
        return ResponseEntity.ok(response);
    }
